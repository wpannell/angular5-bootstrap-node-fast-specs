...
const responseSpecification = eachLike({
  ...
  status: term({
    ...
    matcher: 'PENDING_REVIEW|IN_PROGRESS|COMPLETE|APPROVED|DECLINED'
  }),
...
const responseCode200 = {
  ...
  uponReceiving: 'a get request for access requests',
  withRequest: {
    method: 'GET',
    path: endpoint,
    query: {
  ...
  describe('fetches access requests', () => {
    ...
    describe('with a 200 server response', () => {
      ...
      context('a single request contains a', () => {
        ...
        it('type', () => {
          request.type.should.equal('ACCESS');
        });
